...
def test_index(client):
    res = client.get("/")
    assert "Voting" in res


def test_change_language(app, client):
    res = client.post(
        "/change_language", {"lang": "de", "back_url": "http://localhost/"}, status=302
    )
...
def test_change_language_rejects_rouge_redirect(app, client):
    client.post(
        "/change_language",
        {"lang": "de", "back_url": "http://bad.example.com/"},
        status=400,
    )
...
def test_change_language_url_with_params(app, client):
    res = client.post(
        "/change_language",
        {"lang": "en", "back_url": "http://localhost/fromurl?param=yes&param2=no"},
        status=302,
    )
