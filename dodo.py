...
SASS_PATHS = [BASEDIR / "sass" / "voting.sass", BASEDIR / "sass" / "_custom.sass"]

DOIT_CONFIG = {"default_tasks": ["babel_compile", "css_compile"]}
...
def task_babel_compile():
    return {
        "actions": [f"pybabel compile -d {TDIR}"],
        "file_dep": PO_PATHS,
        "targets": MO_PATHS,
        "clean": True,
    }
...
def task_css_compile():
    ...
    return {
        "actions": [
            f"sassc -I {sass_path} src/ekklesia_voting/sass/voting.sass %(targets)s"
        ],
        "file_dep": SASS_PATHS,
        "targets": [BASEDIR / "static" / "css" / "voting.css"],
        "clean": True,
    }
...
def task_babel_init():
    return {
        "actions": [f"pybabel init -i {POT_PATH} -d {TDIR} -l %(lang)s"],
        "params": [
            {
                "name": "lang",
                "default": "",
...
def task_babel_extract():
    return {"actions": [f"pybabel extract -F babel.cfg -o {POT_PATH} {SRC_PATHS}"]}
...
def task_babel_update():
    return {"actions": [f"pybabel update -d {TDIR} -i {POT_PATH}"]}
