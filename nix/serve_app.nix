...
let
  ...
  exportConfigEnvVar =
    lib.optionalString
      (appConfigFile != null)
      "export EKKLESIA_VOTING_CONFIG=\${EKKLESIA_VOTING_CONFIG:-${appConfigFile}}";
  ...
  runGunicorn = pkgs.writeShellScriptBin "ekklesia-voting-serve-app" ''
    ${exportConfigEnvVar}
    ${lib.optionalString (tmpdir != null) "export TMPDIR=${tmpdir}"}

    ${gunicorn}/bin/gunicorn -c ${gunicornConf} \
      "ekklesia_voting.app:make_wsgi_app()"
  '';
