...
def app_setting_section():
    return {
        ...
        "login_visible": False,
        "report_url": None,
        "source_code_url": "https://github.com/edemocracy/ekklesia-voting",
        "title": "Ekklesia Voting Dev",
        "tos_url": None,
    }
...
def create_or_update_user(request, ekklesia_auth: EkklesiaAuth) -> None:
    ...
    if (
        required_role_for_login is not None
        and required_role_for_login not in userinfo.roles
    ):
        request.flash(
            _(
        ...
        request.flash(
            _("alert_logged_in_to", name=auth_settings.display_name), "success"
        )
    ...
    departments = (
        request.q(Department).filter(Department.name.in_(userinfo.roles)).all()
    )
...
def get_app_settings(settings_filepath=None):
    ...
    if settings_filepath is None:
        ...
        logg.warning("config file path %s doesn't exist!", settings_filepath)
...
def get_locale(request):
    ...
    if locale:
        ...
        locale = request.accept_language.best_match(
            request.app.root.settings.app.languages
        )
